...
def _build(tmp_path, samples, rows, **kw):
    ...
    # 夹具多是 K=2(造 4 条会让用例读不动),而生产默认 min_samples=4 = 采样 K:
    # 显式把旋钮调到夹具的 K,免得每个用例都在测「条数不够」
...
def test_classify_thresholds():
    ...
    # 默认 min_samples = 采样 K(4):只落了 1~2 条的残缺题不许拿单条错答判「不会」
    assert ba.DEFAULT_MIN_SAMPLES == 4
    assert ba.classify(_flags(1, 1, 0, 1)) == ("unstable", "u_too_few")
    assert ba.classify(_flags(2, 2, 0, 2)) == ("unstable", "u_too_few")
    # 规则层判不了(开放题语义等价)不是判错:不能当作「不会」
    flags = _flags(4, 4, 0, 4)
    flags[0]["unjudgeable"] = True
    assert ba.classify(flags) == ("unstable", "u_unjudgeable")
...
DOUBLE = "综合以上分析。\n答案:A\n</think>\n\n答案:A"  # 收尾写了两遍(split_answer 的 docstring 记过实测)


def test_double_think_close_is_dropped_not_smuggled_into_the_row(tmp_path):
    """答卷里有两个 </think> 时整条丢掉,不能只按最后一个切。

    只切最后一个的话,第一个收尾标签和它后面的「答案:A」会原样留在教材的思考段里:
    教出来的是「思考中途写 </think>」(而严格口径就靠这个标签),弃权行更会变成
    「先给出一个具体选项、再改口说不确定」——正是这条训练线要消除的行为。
    """
    assert ba.split_think("思考\n</think>\n\n答案:A\n</think>\n\n答案:A") == ("", "")
    # q1 全错 + 全是双收尾 → 四条都判脏,一条证据都不剩 → 不是 unknown
    rows = [_row("q1", k, final=DOUBLE) for k in range(4)]
    # q2 全错、正常收尾 → unknown(用来确认丢的是脏样本而不是整个流程停摆)
    rows += [_row("q2", k, final=WRONG) for k in range(4)]
    stats, lines, _ = _build(tmp_path, [_sample("q1"), _sample("q2")], rows, abstain_ratio=0.5)
    assert stats["counts"]["double_close"] == 4
    assert stats["classes"] == {"known": 0, "unknown": 1, "unstable": 1}
    assert len(lines) == 1
    a = _assistant(lines[0])
    assert a.count("<think>") == 1 and a.count("</think>") == 1 and "答案:A" not in a


def test_double_think_close_in_a_known_question_is_dropped(tmp_path):
    """known 侧同样:双收尾的正解不能进教材(标签不成对)。"""
    right_double = "综合以上分析。\n答案:B\n</think>\n\n答案:B"
    rows = [_row("q1", k, final=right_double) for k in range(4)]
    stats, lines, _ = _build(tmp_path, [_sample("q1")], rows)
    assert stats["counts"]["double_close"] == 4 and lines == []


def test_open_question_unjudgeable_is_not_unknown(tmp_path):
    """规则层对开放题只认精确相等,语义等价一律弃权——「判不了」不能折成「判错」。

    没有这条护栏,模型答对了的开放题(「口服阿莫西林」vs gold「阿莫西林」)会被成批
    写成「教它说不确定」的教材,而分类直方图里看不出任何异常。
    """
    s = Sample(id="o1", source="cmexam-train", question="首选药物是什么?", gold="阿莫西林", options=None)
    rows = [_row("o1", k, final="综合以上分析。\n最终答案:口服阿莫西林") for k in range(4)]
    stats, lines, _ = _build(tmp_path, [s], rows)
    assert stats["classes"] == {"known": 0, "unknown": 0, "unstable": 1}
    assert stats["counts"]["u_unjudgeable"] == 1 and lines == []


def test_cli_exits_when_no_normal_rows(tmp_path):
    """纯弃权教材同样要喊停:downsample 在一类为空时放弃比例控制,会产出 100% 弃权的数据。"""
    kk, uu = ba.downsample([], ["u1", "u2", "u3"], 0.35)
    assert (kk, uu) == ([], ["u1", "u2", "u3"])  # 比例未生效,必须由 main 兜住
    pool = _write_pool(tmp_path, [_sample("u1")])
    sfile = _write_samples(tmp_path, [_row("u1", k, final=WRONG) for k in range(2)])
...
def test_truncated_and_undeclared_are_not_unknown(tmp_path):
    ...
    rows = (
        ...
        [
            _row("q1", 0, final=WRONG),
            _row("q1", 1, final=WRONG),
            _row("q1", 2, final=WRONG, finish="length"),
            _row("q1", 3, final=WRONG, finish="length"),
        ]
        ...
        + [
            _row("q4", 0, final=WRONG),
            _row("q4", 1, final=WRONG),
            _row("q4", 2, final=WRONG),
            _row("q4", 3, final=WRONG, finish="length"),
        ]
    )
...
def test_pool_missing_ids_are_counted_not_crashed(tmp_path):
    stats, lines, report = _build(
        tmp_path, [_sample("q1")], [_row("q1", k) for k in range(4)] + [_row("qX", 0)]
    )
    assert stats["counts"]["pool_missing"] == 1 and len(lines) == 1
    # 分类占比的分母是**已分类**的题,不是采样文件里的全部题:否则三行加起来不到 100%
    assert stats["n_questions"] == 2 and stats["n_classified"] == 1
    assert stats["counts"]["pool_missing_q"] == 1
    assert "| known | 1 | 100.0% |" in report
    assert "另有 1 道题的 id 不在题池里" in report
...
def test_too_long_rows_are_dropped_in_data_layer(tmp_path):
    ...
    stats, lines, _ = _build(
        tmp_path, [_sample("q1")], [_row("q1", k) for k in range(2)], train_max_length=50
    )
...
def test_general_replay_mix(tmp_path):
    ...
    general = [
        {"messages": [{"role": "user", "content": f"通用{i}"}, {"role": "assistant", "content": "好"}]}
        for i in range(50)
    ]
    stats, lines, report = _build(
        tmp_path,
...
def test_report_matches_dataset(tmp_path):
    ...
    rows = (
        [_row(f"k{i}", k) for i in range(4) for k in range(2)]
        + [_row("u1", k, final=WRONG) for k in range(2)]
        + [_row("p1", 0), _row("p1", 1, final=WRONG)]
    )  # 半对半错 → unstable
    ...
    assert "占已分类题" in report and "每题采样条数分布" in report
