...
| + 弃权 SFT(自采样分会/不会,不会的题教「答案:不确定」,零 API 费) | 74.2%(覆盖 91%,答了的题 81.8%) | 60.0%(覆盖 91%,答了的题 66.0%) | 22.1%(覆盖 91%,答了的题 24.5%) | 1.6k / 3.0k / 10.1k |
| + GRPO(可验证奖励:答对 +1、弃权 0、答错 −1;只练难题,100 步) | 78.4%(覆盖 96%,答了的题 81.7%) | 63.9%(覆盖 95%,答了的题 67.4%) | 24.7%(覆盖 95%,答了的题 25.9%) | **1.0k / 1.6k / 4.3k** |
...
**训练线第三阶段(GRPO,配置已就绪待跑)**:蒸馏 SFT 教会「怎么答」、弃权 SFT 教会「答案:不确定」这条字面收尾,GRPO 要教的是**什么时候该用它**。做法是可验证 RL:题池取一段与前两阶段完全不重叠的 CMExam 训练题(同 seed 洗牌后跳过弃权阶段吃掉的前 4000),提示词与评测逐字节相同,奖励由本项目自己的验证器现算——答对 +1、主动弃权 0、答错 -1、没写完或没做出声明 -1。这四个数不是随手定的:期望收益 2p-1 与弃权的 0 恰在 p(答对)=0.5 相交,所以「不确定就别猜」在奖励表里是**算出来的**最优策略,而不是像 P2 那条「允许写不确定」的提示词臂那样求模型自觉(它没有自觉)。未收尾与答错同罚,是为了堵住 DPO 实测过的退化方向——一旦「写不完」比「答错」便宜,模型就会学会永远不收尾。题单还多过一道难度筛:随机切题时 ms-swift 的 `frac_reward_zero_std` 显示大半的 prompt 组里 8 个 rollout 同分(模型全对或全错)、组内优势恒为 0,于是改成从弃权阶段的自采样结果里只挑「4 次采样对 1~3 次」的题——那批题正是 `build_abstain` 整类丢弃的 unstable,一行都没进过教材,拿来做 RL 既不重叠也不浪费(`build_grpo --samples`)。落地在 `configs/grpo_qwen35_4b_lora.yaml`(单卡 32 GB,LoRA + colocate rollout)与 `src/medforge/train/grpo_reward.py`(注册进 ms-swift 的 `medforge_selective` 奖励插件),评测跑默认与弃权两条提示词臂,只差提示词这一个变量。
...
src/medforge/
  ...
  train/            GRPO 奖励插件(由 GPU 机上的 ms-swift 加载执行)
  tools/            (二期)医疗工具调用
  serve/            (W3)部署、压测、监控
...
- [x] 弃权 SFT(✅ 训完:学会了「不确定」这个动作,弃权精度只有随机的 1.9 倍,时机没学会——见 [reports/abstain.md](reports/abstain.md))
- [ ] 蒸馏 2.0 上线(live 模式 / 部署配置)· 多 seed
- [x] 第三阶段 GRPO(✅ 100 步跑完:弃权率减半、总分回到蒸馏 2.0 的噪声范围,弃权精度仍只有随机的 1.5 倍——弃权路线到此为止,主模型仍是蒸馏 2.0,见 [reports/grpo.md](reports/grpo.md))
