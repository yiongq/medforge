...
**训练线第三阶段(GRPO,配置已就绪待跑)**:蒸馏 SFT 教会「怎么答」、弃权 SFT 教会「答案:不确定」这条字面收尾,GRPO 要教的是**什么时候该用它**。做法是可验证 RL:题池取一段与前两阶段完全不重叠的 CMExam 训练题(同 seed 洗牌后跳过弃权阶段吃掉的前 4000),提示词与评测逐字节相同,奖励由本项目自己的验证器现算——答对 +1、主动弃权 0、答错 -1、没写完或没做出声明 -1。这四个数不是随手定的:期望收益 2p-1 与弃权的 0 恰在 p(答对)=0.5 相交,所以「不确定就别猜」在奖励表里是**算出来的**最优策略,而不是像 P2 那条「允许写不确定」的提示词臂那样求模型自觉(它没有自觉)。未收尾与答错同罚,是为了堵住 DPO 实测过的退化方向——一旦「写不完」比「答错」便宜,模型就会学会永远不收尾。落地在 `configs/grpo_qwen35_4b_lora.yaml`(单卡 32 GB,LoRA + colocate rollout)与 `src/medforge/train/grpo_reward.py`(注册进 ms-swift 的 `medforge_selective` 奖励插件),评测跑默认与弃权两条提示词臂,只差提示词这一个变量。
